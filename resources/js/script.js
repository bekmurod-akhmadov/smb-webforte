...
// Универсальная безопасная выборка значения
...
// Универсальная безопасная выборка значения
  ...
  function validateRecipientStep() {
    const name = safeVal("#recipient-name");
    const phone = safeVal("#recipient-phone");
  ...
  function validateSenderStep() {
    const name = safeVal("#sender-name");
    const surname = safeVal("#sender-surname");
    const phone = safeVal("#sender-phone");
    const email = safeVal("#sender-email");
    const terms = document.querySelector("#terms")?.checked || false;
...
function validateForm() {
  ...
  const firstName = safeVal("#firstName");
  const lastName  = safeVal("#lastName");
  ...
  if (document.querySelector(".city-fields")?.offsetParent !== null) {
    const activeBtn = document.querySelector(".delivery-btn.active");
    const deliveryType = activeBtn ? activeBtn.dataset.type : null;
    const serviceSelected = document.querySelector('input[name="deliveryService"]:checked') !== null;
    ...
    if (deliveryType === "courier") {
      const zipCode = safeVal("#zipCode");
      const address = safeVal("#address");
      if (!zipCode || !address) {
        return false;
      }
    } else if (deliveryType === "pickup") {
      const pickup = safeVal("#pickupLocation") || document.querySelector("#selectedPickup")?.offsetParent !== null;
      if (!pickup) {
        return false;
      }
    }
...
$(document).ready(function () {
  ...
  $(document).ready(function () {
    document.querySelectorAll(".profile-modal #phone").forEach((input) => {
  ...
  document.querySelectorAll(".profile input[type='tel']").forEach((input) => {
    Inputmask({ mask: "+7 (999) 999-99-99" }).mask(input);
  });
