...
$(document).ready(function () {
  ...
  $(document).ready(function () {
    ...
    function validatePhoneForm() {
      ...
      const checkbox = document.querySelector(".custom_checkbox_input");
      const nextBtn = document.querySelector(".btn_phone_next");

      const phoneVal = phoneInput?.value.trim() || "";
      const phoneValid = /^\+7\s?\(\d{3}\)\s?\d{3}-\d{2}-\d{2}$/.test(phoneVal);
...
// Подписываем события
    document.querySelector("#phone")?.addEventListener("input", validatePhoneForm);
    document
      .querySelector(".custom_checkbox_input")
      ?.addEventListener("change", validatePhoneForm);
